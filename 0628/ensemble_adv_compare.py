...
#         # OOF: 只对val_idx做TTA
#         for idx in val_idx:
#             aug_texts = tta_augment_texts(X[idx], n_aug=4)
#             probs = []
#             for aug in aug_texts:
#                 enc = tokenizer(aug, padding=True, truncation=True, max_length=256, return_tensors="pt")
#                 with torch.no_grad():
#                     for k in enc:
...
#                     probs.append(prob)
#             tta_oof[idx] = np.mean(probs)
#         # TEST: 每折模型对全部test_texts做TTA，最后对所有折平均
#         test_probs = np.zeros(len(test_texts))
#         for i, text in enumerate(test_texts):
#             aug_texts = tta_augment_texts(text, n_aug=4)
#             probs = []
#             for aug in aug_texts:
#                 enc = tokenizer(aug, padding=True, truncation=True, max_length=256, return_tensors="pt")
#                 with torch.no_grad():
...
#         tta_test_fold_probs.append(test_probs)
...
# 生成各个单独模型的预测结果
for model_tag in model_probs.keys():
    ...
    save_predictions_to_txt(model_test_probs[model_tag], model_pred_path)
    save_probabilities_to_txt(model_test_probs[model_tag], model_prob_path)

# # 生成对抗模型的预测结果
# for model_tag in adv_model_probs.keys():
...
#     save_predictions_to_txt(adv_model_test_probs[model_tag], model_pred_path)
#     save_probabilities_to_txt(adv_model_test_probs[model_tag], model_prob_path)

# # 生成TTA模型的预测结果
# for model_tag in tta_model_probs.keys():
...
#     save_predictions_to_txt(tta_model_test_probs[model_tag], model_pred_path)
#     save_probabilities_to_txt(tta_model_test_probs[model_tag], model_prob_path)
