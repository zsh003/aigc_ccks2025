...
log_file = os.path.join(base_dir, f"train_predict_{model_para_scale}_{ft_method}-fv_{version}.log")
...
class AIGCDetectionDataset(Dataset):
    def __init__(self, data, tokenizer, max_length=256):
        ...
        # 定义多个 prompt 模板，用于 prompt-level 数据增强
        self.prompt_templates = [
            "请判断以下文本是由 AI 自动生成的，还是由真实人类撰写的？\n{text}",
            "你是一位AI内容检测专家，请分析下面的文本内容，并判断其来源。\n{text}",
            "以下文本是 AI 写的还是人类写的？请仔细判断。\n{text}",
            "判断文本是人类撰写还是由AI生成，请基于语义连贯性与风格给出判断：\n{text}",
            "你是一名训练有素的语言学者，请判断以下内容的来源。\n{text}",
        ]
...
def main():
    ...
    def augment_text(text):
        ...
        if random.random() < 0.3:
            ...
            if len(sentences) > 2:
                text = '.'.join(sentences[:random.randint(2, len(sentences))]) + '.'
        return text
    ...
    if is_model_saved(OUTPUT_DIR):
        ...
        multiprocess_predict(
            ...
            num_workers=32  # 可根据显卡资源调整，如16、32等
        )
        print("全部流程结束！")
        return
    print("加载模型和分词器...")
    ...
    training_args = TrainingArguments(
        ...
        learning_rate=2e-5,                    # 初始略大，配合 warmup
        lr_scheduler_type="cosine",           # 更平滑衰减
        weight_decay=0.01,
        max_grad_norm=1.0,
        warmup_steps=50,                     # 防止前期更新过猛
        #early_stopping_patience=3, #TypeError: TrainingArguments.__init__() got an unexpected keyword argument 'early_stopping_patience'
        logging_steps=50,
        save_strategy="no",
        #save_strategy="steps",
        #save_steps=200,
    ...
    print("清理训练资源...")
    # 训练后记录实验结果
    last_log = trainer.state.log_history[-1] if hasattr(trainer, 'state') and trainer.state.log_history else {}
    train_loss = last_log.get('train_loss', last_log.get('loss'))
    train_runtime = last_log.get('train_runtime')

    exp_metrics = {
        "train_loss": float(train_loss) if train_loss is not None else None,
        "train_runtime": float(train_runtime) if train_runtime is not None else None,
    }
    def save_experiment_log(params, metrics, log_dir="experiment_logs"):
        ...
        log_path = os.path.join(log_dir, f"exp_{log['experiment_id']}.json")
        with open(log_path, "w", encoding="utf-8") as f:
            json.dump(log, f, ensure_ascii=False, indent=2)
        print(f"实验日志已保存: {log_path}")
    save_experiment_log(exp_params, exp_metrics)
    
    # 显式删除trainer和model，并强制垃圾回收
    del trainer
    del model
    del base_model
    clear_all_resources() # 调用清理函数，确保子进程和CUDA缓存被清理
