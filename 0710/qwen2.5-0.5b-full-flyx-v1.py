...
# 检查环境变量设置
print(f"当前HF_ENDPOINT: {os.environ.get('HF_ENDPOINT', '未设置')}")
print(f"当前TRANSFORMERS_VERBOSITY: {os.environ.get('TRANSFORMERS_VERBOSITY', '未设置')}")

# 定义路径 - 更新为Qwen2.5模型路径
MODEL_NAME = "Qwen/Qwen2.5-0.5B"  # 更新为Qwen2.5路径
TRAIN_PATH = "../datasets/train/train.jsonl"
TEST_PATH = "../datasets/test_521/test.jsonl"
OUTPUT_DIR = "../0710/full_finetuned_model_0.5B_v1"  # 更新输出目录名
RESULT_PATH = "../0710/submit_full_finetune_0.5B_v1.txt"  # 更新结果文件名
CHECKPOINT_DIR = "../0710/checkpoints_full_finetune_0.5B_v1"  # 更新检查点目录
...
HF_CACHE_DIR = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "..", "..", ".hf_cache", "hub"
)
...
class AIGCDetectionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if "label" in item:
            ...
            # 对assistant部分进行编码
            assistant_encodings = self.tokenizer(
            ...
            prompt_tokens = self.tokenizer(
                full_prompt[:assistant_start], add_special_tokens=False
            )
...
def predict_test_data(
    model, tokenizer, test_data, batch_size=8
):  # 减少batch_size以适应全参数微调
    """批量预测以提高速度"""
    predictions = []
    device = model.device
    model.eval()
    ...
    for i in range(0, len(test_data), batch_size):
        ...
        for _, row in batch_data.iterrows():
            ...
            if i < 50:  # 只显示前50个样本的调试信息
                print(
                    f"样本 {i + 1}: 输入长度={input_length}, 输出长度={len(outputs[0])}"
                )
...
def is_model_saved(model_dir):
    ...
    model_files = [
        ...
        os.path.join(model_dir, "pytorch_model-00001-of-00001.bin"),
    ...
    for model_file in model_files:
        if os.path.exists(model_file) and check_model_file_size(model_file):
            print(f"检测到有效模型文件: {model_file}")
            print(f"  - config.json: {config_exists}")
            return True

    print(f"目录存在但缺少有效的模型文件: {model_dir}")
    return False
...
def main():
    ...
    if is_model_saved(OUTPUT_DIR):
        print(f"检测到已保存的模型，直接加载进行预测: {OUTPUT_DIR}")

        # 验证模型完整性
        try:
            # 加载模型和分词器
            print(f"加载模型: {OUTPUT_DIR}")
            model = AutoModelForCausalLM.from_pretrained(
                OUTPUT_DIR, torch_dtype=torch.float16, device_map=None
            )
            ...
            try:
                model = AutoModelForCausalLM.from_pretrained(
                    OUTPUT_DIR, torch_dtype=torch.float16, device_map=None, trust_remote_code=True
                )
                ...
                if os.path.exists(OUTPUT_DIR):
                    ...
                    print(f"已删除不完整的模型目录: {OUTPUT_DIR}")
    ...
    training_args = TrainingArguments(
        ...
        num_train_epochs=1,
        per_device_train_batch_size=1,  # 减少batch_size以适应全参数微调
        per_device_eval_batch_size=1,
        gradient_accumulation_steps=32,  # 增加梯度累积步数
        learning_rate=1e-5,  # 降低学习率以适应全参数微调
        weight_decay=0.01,
        max_grad_norm=1.0,  # 添加梯度裁剪
        warmup_steps=100,  # 增加预热步数
        save_strategy="steps",
        save_steps=500,  # 每500步保存一次
        save_total_limit=2,  # 保留2个检查点以节省空间
        logging_steps=50,
        ...
        dataloader_num_workers=4,  # 减少多进程数据加载
    ...
    print(
        f"可训练参数数量: {sum(p.numel() for p in model.parameters() if p.requires_grad)}"
    )
